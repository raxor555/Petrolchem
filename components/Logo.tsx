...
const Logo: React.FC<{ className?: string }> = ({ className = "h-10 w-10" }) => {
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <svg viewBox="0 0 100 100" className="h-full w-auto" fill="#4B1771">
        {/* Recreating the dotted mandala style logo from the reference */}
      ...
      <span className="text-[20px] md:text-[24px] font-black text-[#4B1771] tracking-[-0.05em] leading-none uppercase">
        PETROLCHEM
      </span>
    </div>
  );
